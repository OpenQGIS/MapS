...
async function loadLayers() {
  try {
    ...
    if (!rawData) {
      const sRes = await fetch("./data/layers.json");
...
function generateClientQgisScript(selectedLayers, addToCanvas) {
  ...
  const hasVpn = selectedLayers.some(l => l.needs_vpn);
  if (hasVpn) {
    lines.push("# -------------------------------------------------------------");
    lines.push("# 💡 网络代理提醒：");
    lines.push("# 本批次底图中包含境外源。若在 QGIS 画布中加载显示红叉或空白，");
    lines.push("# 请在 QGIS 菜单栏【设置】->【选项】->【网络】中配置本地网络代理（如 127.0.0.1:7890）。");
    lines.push("# -------------------------------------------------------------");
    lines.push("");
  }
